...
Since the dissertation focuses on restructuring a report, it does not tackle a specific research question. Instead, the main objective of this work is to develop an independent program that automates the triage of security alerts within a Security Operations Center (SOC) environment. This automation aims to enhance the overall efficiency and accuracy of the triage process, reduce the number of false positives, and alleviate the workload during the initial stages of the SOC workflow. To successfully achieve this objective, a series of specific goals need to be followed:

\begin{itemize}
    \item Choose an existing ML model for categorizing security tickets based on historical data, correctly estimating its priority and taxonomy.
    \item Standardize data preprocessing for consistent alert structure, enabling accurate ML classifications.
    \item Integrate the application with a SIEM system and other data sources.
    \item Assess the model's effectiveness in reducing false positives and false negatives.
    \item Improve productivity by enabling a focus on critical incidents.
    \item Ensure the solution adapts and learns constantly from new data.
\end{itemize}
