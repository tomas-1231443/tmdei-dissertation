...
\begin{itemize}
    \item \textbf{Parallel Ensembles}: Base learners are trained independently on different subsets of data or features. Techniques like bagging and Random Forests fall into this category \parencite{Breiman1996}.
    \item \textbf{Sequential Ensembles}: Models are trained iteratively, with each learner focusing on correcting the errors of its predecessor. Boosting is the most prominent example of this approach \parencite{Schapire1990}.
...
The Random Forest algorithm begins by generating multiple decision trees, each trained on a random subset of the training data through bootstrapping\footnote{Bootstrapping is a resampling technique where random samples are drawn with replacement from the dataset.}.
...
In practice, the Naive Bayes model estimates the prior probability \( P(C) \) for each class and the conditional probability \( P(X_i|C) \) for each feature \( X_i \) given the class. 
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.5\textwidth]{ch2/assets/Flow-chart-for-Naive-Bayesian-classification.png}
...
Their work effectively alleviated analyst fatigue by prioritizing actionable alerts, yet the scope of implementation remains limited to isolated systems.
...
QRadar is an industry-leading \gls{SIEM} solution developed by IBM. 
It provides advanced threat detection and automated response capabilities. 
QRadar aggregates and correlates log data from multiple sources, enabling analysts to detect anomalies, identify risks, and prioritize responses efficiently.

\subsubsection{Splunk}
Splunk is a robust \gls{SIEM} platform designed to analyze and visualize machine data. 
Its real-time search and analytics features make it highly effective for monitoring, detecting, and responding to cybersecurity threats. 
Splunk's extensive ecosystem of apps enhances its functionality for various use cases.

\subsubsection{LogRhythm}
LogRhythm is a comprehensive \gls{SIEM} solution focused on streamlining threat detection and response workflows. 
With advanced analytics and machine learning integration, LogRhythm enables organizations to quickly identify and remediate security incidents while maintaining compliance.

\subsection{Ticketing Tools}
\subsubsection{IBM QRadar SOAR}
IBM QRadar SOAR integrates seamlessly with QRadar SIEM to enhance incident management. 
It offers playbook automation, case management, and collaboration tools, enabling analysts to streamline their workflows and improve response times.

\subsubsection{ServiceNow}
ServiceNow is a widely used platform for IT service management that extends its capabilities to incident management in cybersecurity. 
Its integration with various tools and ability to automate workflows make it a valuable asset in SOC environments.

\subsubsection{Zendesk}
Zendesk is a versatile ticketing and customer support tool that can be adapted for managing security incidents. 
Its user-friendly interface and integration capabilities simplify the management and tracking of security alerts.

\subsection{Machine Learning Frameworks}
\subsubsection{Scikit-learn}
Scikit-learn is a Python-based machine learning library well-suited for implementing Random Forest and Naive Bayes algorithms. 
It provides a wide range of tools for data preprocessing, model training, and evaluation.

\subsubsection{TensorFlow}
TensorFlow is an open-source machine learning framework developed by Google. 
Known for its scalability and flexibility, TensorFlow supports both deep learning and traditional machine learning methods, including ensemble techniques like Random Forest.

\subsubsection{PyTorch}
PyTorch is another popular machine learning library offering dynamic computation graphs and extensive support for research and deployment. 
Its ease of use and flexibility make it a strong choice for implementing complex machine learning models in cybersecurity applications.

\subsection{Comparative Analysis}
\subsubsection{SIEM Tools}
QRadar stands out for its deep integration with IBM solutions, making it ideal for organizations already using IBM's ecosystem. 
Splunk excels in real-time analytics and offers extensive third-party integrations, making it suitable for diverse use cases. 
LogRhythm provides strong machine learning capabilities and focuses on compliance, but may lack the seamless integration offered by QRadar.

\subsubsection{Ticketing Tools}
IBM QRadar SOAR integrates tightly with QRadar, enabling efficient workflows for organizations using IBM products. 
ServiceNow is highly customizable and supports broader IT service management, making it ideal for multi-department organizations. 
Zendesk is user-friendly and cost-effective but may not offer the depth of automation provided by IBM QRadar SOAR.

\subsubsection{Machine Learning Frameworks}
Scikit-learn is lightweight and highly effective for implementing algorithms like Random Forest and Naive Bayes, making it the best choice for projects focused on classical machine learning. 
TensorFlow offers scalability and is better suited for deep learning applications, while PyTorch provides flexibility and is highly popular in research environments.
