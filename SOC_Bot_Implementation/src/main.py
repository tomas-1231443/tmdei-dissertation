...
def train(path):
    ...
    try:
        ...
        logger.debug(f"Loaded Excel data with shape: {df.shape}")
        ...
        return
    return train_rf_model(df_clean, label_col="Status")

@click.command()
@click.option("-v", "--verbose", is_flag=True, default=False, help="Enable verbose (DEBUG) logging.")
@click.option("--excel-path", default=DEFAULT_EXCEL_PATH, help="Path to the Excel file with historical alerts.")
...
def main(verbose, excel_path, retrain, model_version, port):
    """
    ...
    logger.debug(f"Arguments received - verbose: {verbose}, excel_path: {excel_path}, retrain: {retrain}, model_version: {model_version}, port: {port}")
